# HYO Email Project Agent Rules (Codex & Antigravity)

이 저장소(`hyo_email`)에서 작업하는 모든 AI 에이전트(Codex, Antigravity 등)는 다음 규칙과 파이프라인을 준수해야 합니다.
...
## 2. 서버 및 실행 환경 유의사항
- **파이썬 실행 경로:** 의존성이 설치된 `C:\Python37\python.exe`를 사용하여 서버 및 스크립트를 실행합니다.
  ```bash
